...
def run_experiment():
    long_text = """
    Revenue grew 18% YoY following expansion into enterprise accounts. 
    However, churn increased from 3.1% to 4.0% after a pricing change 
    that simplified tiers but increased costs for mid-market customers.

    Support tickets rose 25% over the same period, with the majority 
    related to billing confusion and feature access restrictions. 
    Customer sentiment in surveys shifted slightly negative in mid-tier accounts.

    Enterprise ARPA improved significantly due to bundled contracts, 
    but smaller customers showed higher downgrade and cancellation rates. 
    Marketing spend also increased 15% during this period.
    """
    prompt = (
    ...
    # Save summary
    with open(OUTDIR / "summary_4.json", "w") as f:
